...
> 本仓库根目录包含：完整代码（[`PaperAgent-main/`](./PaperAgent-main)）+ 比赛/交付文档（`p0交付文档.md`、`阶段二~八交付文档.md`、`待完成.md`、方案 .docx）。以下为项目说明，代码相关的路径均以 `PaperAgent-main/` 为其根。
...
│   ├── 快速启动.md               # 组员安装与启动说明
│   └── README.md
├── p0交付文档.md / 阶段二~八交付文档.md / 待完成.md   # 交付与进度文档
└── 本 README
```

## 本地运行

> 代码均在 [`PaperAgent-main/`](./PaperAgent-main) 下，下文命令请先 `cd PaperAgent-main`。

### 一键启动（推荐）

在代码根目录执行 `start_all.ps1`，脚本会自动依次启动后端 API（`main.py`，`:8001`）、向量化 Worker（`vector.py`）和前端（Vite，`:8080`）：
...
> 首次部署请先阅读 [快速启动.md](./PaperAgent-main/快速启动.md)，按步骤安装 Node.js、创建 `.venv` 并安装依赖、`npm install`。脚本会自动检测缺失项并给出提示。

### 1. 后端配置

```powershell
cd PaperAgent-main/PaperQuery_Backend
python -m venv .venv
...
```powershell
cd PaperAgent-main/PaperQuery_Frontend
npm install
...
到 `PaperAgent-main/PaperQuery_Backend/models/` 下，目录名与默认配置一致：
...
- 仓库未包含 `models/`、`.venv/`、`node_modules/` 等体积/生成文件，首次运行需按 [快速启动.md](./PaperAgent-main/快速启动.md) 重建。
...
`PaperAgent-main/PaperQuery_Backend/eval/` 下提供三套对比系统的评测框架（`llm_only` / `basic_rag` / `paperagent_full`），含 48 条种子数据集、构建/运行/评分三个脚本。详见 [eval/README.md](./PaperAgent-main/PaperQuery_Backend/eval/README.md)。
